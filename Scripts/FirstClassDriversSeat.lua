...
FirstClassDriversSeat = class( Seat )
FirstClassDriversSeat.maxChildCount = 20
FirstClassDriversSeat.connectionOutput = sm.interactable.connectionType.seated + sm.interactable.connectionType.power + sm.interactable.connectionType.bearing
FirstClassDriversSeat.colorNormal = sm.color.new( 0x80ff00ff )
FirstClassDriversSeat.colorHighlight = sm.color.new( 0xb4ff68ff )

local SpeedPerStep = 1 / math.rad( 27 ) / 3

function FirstClassDriversSeat.server_onCreate( self )
...
function FirstClassDriversSeat.server_onFixedUpdate( self )
...
function FirstClassDriversSeat.client_onClientDataUpdate( self, params )
	self.owner = params.owner
	self.name = params.name
end

function FirstClassDriversSeat.client_canInteract( self, character, state )
...
function FirstClassDriversSeat.client_onInteract( self, character, state )
	if state then
		self:cl_seat()
...
function FirstClassDriversSeat.client_canInteractThroughJoint( self )
...
function FirstClassDriversSeat.client_onAction( self, controllerAction, state )
	if state == true then
...
function FirstClassDriversSeat.client_getAvailableChildConnectionCount( self, connectionType )
...
function FirstClassDriversSeat.client_onInteractThroughJoint( self, character, state, joint )
...
function FirstClassDriversSeat.client_onCreate( self )
...
function FirstClassDriversSeat.client_onFixedUpdate( self, timeStep )
...
function FirstClassDriversSeat.client_onUpdate( self, dt )
...
function FirstClassDriversSeat.cl_onLeftAngleChanged( self, sliderName, sliderPos )
	self.cl.updateSettings.leftAngle = sliderPos + 1
	self.cl.updateDelay = 0.1
end

function FirstClassDriversSeat.cl_onRightAngleChanged( self, sliderName, sliderPos )
	self.cl.updateSettings.rightAngle = sliderPos + 1
	self.cl.updateDelay = 0.1
end

function FirstClassDriversSeat.cl_onLeftSpeedChanged( self, sliderName, sliderPos )
	self.cl.updateSettings.leftSpeed = ( sliderPos + 1 ) * SpeedPerStep
	self.cl.updateDelay = 0.1
end

function FirstClassDriversSeat.cl_onRightSpeedChanged( self, sliderName, sliderPos )
	self.cl.updateSettings.rightSpeed = ( sliderPos + 1 ) * SpeedPerStep
	self.cl.updateDelay = 0.1
end

function FirstClassDriversSeat.cl_onLockButtonClicked( self, buttonName )
	self.cl.updateSettings.unlocked = buttonName == "Off"
	self.cl.updateDelay = 0.1
end

function FirstClassDriversSeat.cl_onGuiClosed( self )
	if self.cl.updateDelay > 0.0 then
		self:cl_applyBearingSettings()
		self.cl.updateSettings = {}
		self.cl.updateDelay = 0.0
...
function FirstClassDriversSeat.cl_applyBearingSettings( self )
...
function FirstClassDriversSeat.cl_updateBearingGuiValues( self )
...
function FirstClassDriversSeat.client_canTinker( self, character, state )
	if self.owner == 0 then
		return true
	end
	return false
...
function FirstClassDriversSeat.client_onTinker( self, character, state )
	if state == true then
		self.network:sendToServer("sv_claim")
	end	
end

function FirstClassDriversSeat.sv_claim( self, params, player )
...
function FirstClassDriversSeat.client_canErase(self)
...
function FirstClassDriversSeat.server_canErase(self)
...
function FirstClassDriversSeat.client_playSound(self, name)
	sm.audio.play(name, self.shape.worldPosition)
end

function FirstClassDriversSeat.cl_onAlert( self, params )
