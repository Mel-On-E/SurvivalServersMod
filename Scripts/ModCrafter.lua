...
g_craftingRecipes = {}
g_craftingRecipes["modbot"] = {path = "$SURVIVAL_DATA/CraftingRecipes/craftbot.json", recipes = {}}
local success, data = pcall(sm.json.open, "$MOD_DATA/Scripts/modcraftbot.json")
if success and type(data) == "table" then
	g_craftingRecipes["modbot"].recipes = data
	g_craftingRecipes["modbot"].recipesByIndex = data
end


local crafters = {
	["111cfff0-dfc2-4da7-acdb-3c385e80b2e1"] = {
		needsPower = false,
		slots = 2,
		speed = 1,
		recipeSets = {
		...
		upgrade = "222cfff0-dfc2-4da7-acdb-3c385e80b2e1",
		upgradeCost = 5
	},
	["222cfff0-dfc2-4da7-acdb-3c385e80b2e1"] = {
		needsPower = false,
		slots = 4,
		speed = 1,
		recipeSets = {
		...
		subTitle = "#{LEVEL} 2",
		createGuiFunction = sm.gui.createCraftBotGui,
		upgrade = "333cfff0-dfc2-4da7-acdb-3c385e80b2e1",
		upgradeCost = 5
	},
	["333cfff0-dfc2-4da7-acdb-3c385e80b2e1"] = {
		needsPower = false,
		slots = 6,
		speed = 1,
		recipeSets = {
		...
		subTitle = "#{LEVEL} 3",
		createGuiFunction = sm.gui.createCraftBotGui,
		upgrade = "444cfff0-dfc2-4da7-acdb-3c385e80b2e1",
		upgradeCost = 5
	},
	["444cfff0-dfc2-4da7-acdb-3c385e80b2e1"] = {
		needsPower = false,
		slots = 8,
		speed = 1,
		recipeSets = {
		...
		subTitle = "#{LEVEL} 4",
		createGuiFunction = sm.gui.createCraftBotGui,
		upgrade = "555cfff0-dfc2-4da7-acdb-3c385e80b2e1",
		upgradeCost = 20
	},
	["555cfff0-dfc2-4da7-acdb-3c385e80b2e1"] = {
		needsPower = false,
		slots = 8,
		speed = 2,
		recipeSets = {
...
function Crafter.cl_updateRecipeGrid( self )
	...
	print(self.crafter.recipeSets)
	for _, recipeSet in ipairs( self.crafter.recipeSets ) do
		self.cl.guiInterface:addGridItemsFromFile( "RecipeGrid", "$CONTENT_3f7f4e0f-dc99-43a9-9a15-f7ccbaf7dd65/Scripts/modcraftbot.json", { locked = false } )
	end
end
	...
	if not self.crafter.needsPower or ( parent and parent.active ) then
		...
		for idx, val in ipairs( self.sv.craftArray ) do
			if val then
				...
				local recipeCraftTime = math.ceil( recipe.craftTime*40 / self.crafter.speed ) + 120 -- 1s windup + 2s winddown
	...
	if not self.crafter.needsPower or ( parent and parent.active ) then
		...
		for idx = 1, self.crafter.slots do
			...
			if val then
				...
				local recipe = val.recipe
				local recipeCraftTime = math.ceil( recipe.craftTime*40 / self.crafter.speed ) + 120
			...
			if hasInputContainers then
				...
				for _, container in ipairs( self.sv.pipeGraphs.input.containers ) do
					if consumeCount > 0 then
						consumeCount = consumeCount - sm.container.spend( container.shape:getInteractable():getContainer(), sm.uuid.new(ingredient.itemId), consumeCount, false )
...
function Crafter.cl_n_onCraftFromChest( self, params )
	for _, tbl in ipairs( params ) do
		...
		self.cl.pipeEffectPlayer:pushShapeEffectTask( shapeList, sm.uuid.new(tbl.itemId) )
	end
end
...
function Crafter.sv_n_collect( self, params, player )
	...
	if val then
		local recipe = val.recipe
		if val.time >= math.ceil( recipe.craftTime*40 / self.crafter.speed ) then
